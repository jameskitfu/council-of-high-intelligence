...
<p align="center">
  22 位 AI 人格跨多个大语言模型提供商，共同审议你最棘手的决策。只需一条命令。
</p>
...
> **为什么不直接问 Claude？** 单个提示只能得到一个模型自信的最佳猜测。议会会从不同思想传统中生成 3–22 份独立分析，迫使它们相互质疑，并综合出一个不会掩盖分歧的裁决。这就像请教一位顾问与召集整个董事会之间的区别。
...
| `council-rubin` | Rick Rubin（里克·鲁宾） | 创意指导与艺术真实 | opus | 倾听作品想成为什么 |
| `council-jobs` | Steve Jobs（史蒂夫·乔布斯） | 产品品味与整合体验 | opus | 围绕一个清晰承诺塑造完整体验 |
| `council-leonardo` | Leonardo da Vinci（列奥纳多·达·芬奇） | 观察性综合与视觉发明 | opus | 先观察、再描绘、继而理解 |
| `council-krashen` | Stephen Krashen（斯蒂芬·克拉申） | 语言习得与可理解输入 | sonnet | 先通过有意义的输入习得，再要求输出 |
...
- **乔布斯 vs 鲁宾**——整合的产品体验 vs 艺术真实
- **乔布斯 vs 拉姆斯**——连贯的产品承诺 vs 以用户为中心的清晰表达
- **乔布斯 vs 托瓦兹**——端到端体验 vs 实现可行性
- **鲁宾 vs 托瓦兹**——创意真实 vs 制作约束
- **达·芬奇 vs 亚里士多德**——直接观察 vs 稳定范畴
- **达·芬奇 vs 阿达**——视觉发明 vs 形式抽象
- **克拉申 vs 费曼**——自然习得 vs 明确讲解与检验
...
常规使用会自动选择三人 Triad。`--full` 会调用全部 22 位成员，是成本最高的模式。
...
<details>
<summary><strong>预定义 Triad</strong>——31 个针对特定领域的三人组合</summary>
...
| `creative` | Rubin + Leonardo + Rams | 艺术真实 + 发明 + 用户清晰度 |
| `creator` | Rubin + Jobs + Watts | 真实创作 + 产品方向 + 重构问题 |
| `editing` | Rubin + Rams + Feynman | 核心本质 + 清晰度 + 理解检验 |
| `product-vision` | Jobs + Rams + Torvalds | 整合体验 + 用户需求 + 可行性 |
| `launch` | Jobs + Musashi + Machiavelli | 叙事 + 时机 + 激励 |
| `creator-product` | Jobs + Rubin + Karpathy | 包装 + 真实表达 + AI 能力 |
| `invention` | Leonardo + Ada + Feynman | 观察 + 形式化 + 经验检验 |
| `prototype` | Leonardo + Torvalds + Karpathy | 描绘 + 构建 + 经验迭代 |
| `language-learning` | Krashen + Kahneman + Feynman | 习得 + 学习偏差 + 现实检验 |
| `learn-in-public` | Krashen + Leonardo + Rubin | 输入 + 公开学习 + 真实表达 |
| `english-content` | Krashen + Jobs + Rams | 习得价值 + 内容承诺 + 观众清晰度 |
...
包含全部 22 位成员以及上述领域 Triad，最适合广泛审议。
...
面向内容生产闭环的 8 人面板，从可理解的英语输入一直推进到可运行的成果与清晰的受众表达：

- `karpathy`、`torvalds`、`feynman`、`rubin`、`jobs`、`leonardo`、`krashen`、`rams`
- 闭环：可理解的英语输入 → 观察与视觉综合 → 理解 AI → 可运行的成果 → 真实叙事 → 包装 → 受众清晰度
...
- [Gemini CLI](https://github.com/google-gemini/gemini-cli)（使用 Gemini 技能时需要）
